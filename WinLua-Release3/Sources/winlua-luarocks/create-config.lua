...
local pe = assert(loadfile(".\\pe-parser.lua"))()
...
--~ Begin
--~ **********************************************************************************
print(S"LuaRocks $VERSION.x Configuration Manager.\n")
...
if not vars.TREE_ROOT then
  ...
  if vars.LUA_BINDIR:lower():match([[([\/]+bin[\/]*)$]]) then
	--~ print('No rock tree found. placing next to ')
...
f:write(S"    LUA_LIBDIR = [[$LUA_LIBDIR]],\n")
f:write(S"    LUA_INCDIR = [[$LUA_INCDIR]],\n")
