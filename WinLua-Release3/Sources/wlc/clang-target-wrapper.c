 ...
 * This file is part of the WinLua Lua distribution.
 * 
...
//~2020-02-25: rhaley - changed for winlua default 32 bit
#define DEFAULT_TARGET_X64 "x86_64-w64-mingw32"
#define DEFAULT_TARGET "i686-w64-mingw32"
#endif

#ifndef WINLUA_COMPILER
#define WINLUA_COMPILER "wlc"
#define X64 "64"
#define X86 "32"
#define WLC_LEN 3
#define ARCH_LEN 2
...
void check_winlua( const TCHAR **target_ptr, const TCHAR **exe_name_ptr)
{	
	TCHAR *lower = _tcsdup(*exe_name_ptr);
	int str_len = strlen(lower);

	if(str_len < WLC_LEN + ARCH_LEN)
	{
		return;
	}
	...
	if(strncmp(lower, WINLUA_COMPILER, WLC_LEN) == 0)
	{			
		*exe_name_ptr = _tcsdup(CLANG);

		if(strncmp(&lower[WLC_LEN], X86, ARCH_LEN) == 0)
		{
			*target_ptr = _tcsdup(DEFAULT_TARGET);
		}
		else if(strncmp(&lower[WLC_LEN], X64, ARCH_LEN) == 0)
		{
			*target_ptr = _tcsdup(DEFAULT_TARGET_X64);
		}
	}	
}
...
int _tmain(int argc, TCHAR* argv[]) {
    ...
    split_argv(argv[0], &dir, &base_name, &target, &exe);
    check_winlua(&target, &exe);
    if (!target)
