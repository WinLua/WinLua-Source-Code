...
if %LUA_SVER%==54 (
   set FILES_CORE=lapi lcode lctype ldebug ldo ldump lfunc lgc llex lmem lobject lopcodes lparser lstate lstring ltable ltm lundump lvm lzio
   set FILES_LIB=lauxlib lbaselib lcorolib ldblib liolib lmathlib loadlib loslib lstrlib ltablib lutf8lib linit
   set FILES_DLL=lua
   set FILES_OTH=luac
   set INSTALL_H=lauxlib.h lua.h lua.hpp luaconf.h lualib.h
   if "%COMPATFLAG%"=="" (
